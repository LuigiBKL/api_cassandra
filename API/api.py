...
class DataAccess():
    ...
    def connexion(cls):
        ...
        cls.cluster = Cluster(['cassandra-miamiam1','cassandra-miamiam2'], port=9042)
    ...
    def get_info_inspection(cls,idrestaurant):
            ...
            cls.connexion()
            #rows = session.execute('SELECT * FROM restaurant WHERE cuisinetype="Mexican"')
            rows = cls.session.execute(f"SELECT score,idrestaurant,violationdescription  FROM inspection WHERE idrestaurant = {idrestaurant} ;")
            for row in rows:
